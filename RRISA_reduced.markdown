...
All of the existing IGRINS data have been reduced using the [(IGRINS plp)](https://github.com/igrins/plp).
The Reduced version of RRISA includes object names that have been manually corrected by searching through the IGRINS paper logs for handwritten coordinates and names, and through SIMBAD for nomenclature corrections and coordinate offsets.
Targets that are subcomponents of systems or extended sources keep their Raw identifier, and if not SIMBAD searchable they do not have any additional information added in the XMatch component of RRISA.
...
The wavelength solution for PLP reduced data is derived in a multi-step process.
...
This process is not perfect and can leave residual features due to differences in the line widths, and radial velocities of the telluric stars relative to the Vega model. When using PLP data for science pertaining to individual line strengths, you should compare your spectrum to that of the A0V, the Vega model and an atlas of [the Earth transmittance spectrum](https://psg.gsfc.nasa.gov/index.php). If your spectrum has significant telluric residuals, we provide Python based tools on the Tutorial page to help you remove these features. If you spectrum has access A0V residuals, you can use the Vega model and A0V spectrum from the PLP to perform your own telluric correction. Additional A0V observations from the same night might also provide a good correction. 
